do
local Shahab = 119989724 --put your id here(BOT OWNER ID)
...
local function res_user_callback(extra, success, result) -- /info <username> function
  ...
  if result.username then
   ...
   Username = 'ندارد'
    ...
    if not value then
	 if result.id == tonumber(Shahab) then
...
local function action_by_id(extra, success, result)  -- /info <ID> function
 ...
 if result.username then
  ...
  if not value then
	 if result.id == tonumber(Shahab) then
	   text = text..'مقام : مدیر کل ربات (Executive Admin) \n\n'
	  elseif is_admin(result.id) then
	   text = text..'مقام : ادمین ربات (Admin) \n\n'
	  elseif is_spromoted(result.id, extra.chat2) then
	   text = text..'مقام : مدیر کل گروه (Owner) \n\n'
...
local function action_by_reply(extra, success, result)-- (reply) /info  function
	...
	local hash = 'rank:variables'
		local value = redis:hget(hash, result.from.id)
		 if not value then
		    if result.from.id == tonumber(Shahab) then
		       text = text..'مقام : مدیر کل ربات (Executive Admin) \n\n'
		     elseif is_admin(result.from.id) then
		       text = text..'مقام : ادمین ربات (Admin) \n\n'
		     elseif is_spromoted(result.from.id, result.to.id) then
		       text = text..'مقام : مدیر کل گروه (Owner) \n\n'
  ...
  if msg.from.username then
   ...
   local hash = 'rank:variables'
	if hash then
	  ...
	  if not value then
		if msg.from.id == tonumber(Shahab) then
		 text = text..'مقام : مدیر کل ربات (Executive Admin) \n\n'
		elseif is_sudo(msg) then
		 text = text..'مقام : ادمین ربات (Admin) \n\n'
		elseif is_spromoted(msg) then
		 text = text..'مقام : مدیر کل گروه (Owner) \n\n'
